...
const ProfileDashboard: React.FC<ProfileDashboardProps> = ({ user, onClose, onSignOut, onAccountUpdate }) => {
    ...
    useEffect(() => {
        if (showReferrals && user.id) {
            fetch(`http://localhost:3001/api/referrals/${user.id}`)
                .then(res => res.json())
                .then(data => {
                                        ...
                                        <div className="space-y-3 max-h-48 overflow-y-auto pr-2 custom-scrollbar">
                                            {bookings.map((b, idx) => (
                                                <div key={idx} className="flex justify-between items-center bg-white/5 p-3 rounded-xl border border-white/5">
                                                    <div>
                                                        <p className="text-sm font-bold text-white uppercase tracking-wider">{b.package_name}</p>
                                                        <div className="flex items-center gap-3 mt-1">
                                                            <span className="text-[10px] text-indigo-400 font-bold tracking-widest leading-none block">DEST: {b.destination}</span>
                                                            <span className="text-[10px] text-white/40 font-bold tracking-widest leading-none block">DATE: {new Date(b.travel_date).toLocaleDateString()}</span>
                                                        </div>
                                                        <p className="text-[10px] text-emerald-400 font-bold tracking-widest mt-1 leading-none">TOTAL: ₹{b.total_amount}</p>
                                                    </div>
